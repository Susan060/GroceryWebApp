...
function CartPage() {
    ...
    return (
        <div className='w-[95%] sm:w-[90%] md:w-[80%] mx-auto mt-8 mb-24 relative'>
            <Link href={"/"} className='absolute -top-2 left-0 flex items-center gap-2 text-green-800 hover:text-green-950 font-medium transition-all'>
                <ArrowLeft size={20} />
                <span className='hidden sm:inline '>Back to Home</span>
            </Link>
            <motion.h2 initial={{ opacity: 1, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.3 }} className='text-2xl sm:text-3xl md:text-4xl font-bold text-green-700 text-center mb-10'>
                🛒Your Shopping Cart
            </motion.h2>
            ...
            {cartData.length == 0 ? (
                <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ duration: 0.3 }} className='text-center py-20 bg-white rounded-2xl shadow-md'>
                ...
                <div className='grid grid-cols-1 lg:grid-cols-3 gap-8'>
                        ...
                        <AnimatePresence>
                            {cartData.map((item, index) => (
                                ...
                                hover:shadow-xl transition-all duration-300 border border-gray-100 '>
                                    <div className='relative w-28 h-28 sm:h-24  sm:w-24  md:h-28 md:w-28 flex-shrink-0 rounded-xl 
                                    ...
                                    <div className='mt-4 sm:mt-0 sm:ml-4 flex-1 text-center sm:text-left'>
                                        ...
                                        <p className='text-xs sm:text text-gray-500 '>{item.unit}</p>
                                        <p className='text-green-700 font-semibold mt-1 text-sm sm:text-base'>रु{Number(item.price) * item.quantity}</p>
                                    </div>
                                    <div className='flex items-center justify-center sm:justify-end gap-3 mt-3 sm:mt-0 bg-gray-50 px-3 py-2 rounded-full'>
                                        <button className='bg-white p-1.5 rounded-full hover:bg-green-100 transition-all border border-gray-200' onClick={() => dispatch(decreaseQuantity(item._id))}>
                                            <Minus size={14} className='text-green-700' /></button>
                    ...
                    <motion.div initial={{ opacity: 0, x: 30 }}
                        ...
                        <motion.button whileTap={{scale:0.95}} className='w-full mt-6 bg-green-600 text-white py-3 rounded-full hover:bg-green-700 transition-all 
                        font-semibold text-sm sm:text-base' onClick={()=>router.push("/user/checkout")}>
                            Proceed to Checkout
                        </motion.button>
