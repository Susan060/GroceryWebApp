        ...
        <div className='pt-4 w-[95%] md:w-[85%] mx-auto pb-20'>
            ...
            <motion.div
                ...
                <button onClick={() => router.push("/")}
                    ...
                    text-green-700 font-semibold px-4 py-2 rounded-full transition w-full sm:w-auto'><ArrowLeft size={18} /><span>Back</span> </button>
                <h1 className='text-2xl md:text-3xl font-extrabold text-green-700 flex items-center justify-center gap-2'>
                    <Package size={28} className='text-green-600' />Manage Groceries</h1>
            </motion.div>
            {/* Search Bar */}
            <motion.form
                initial={{ opacity: 0, x: -20 }}
                        ...
                        <div className='flex-1 flex flex-col justify-between w-full'>
                            ...
                            <div className='mt-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2'>
                                <p className='text-green-700 font-bold text-lg'>
                                    रु {g.price}/<span className='text-gray-500 text-sm font-medium ml-1'>{g.unit}</span>
                                </p>
                                <button
                                    className='bg-green-600 text-white px-4 py-2 rounded-lg text- font-semibold flex items-center justify-center gap-2 hover:bg-green-700
                                transition-all' onClick={() => { setEditing(g) }}>
                                    <Pencil size={15} /> Edit
                                </button>
                            </div>
                    ...
                    <motion.div
                        ...
                        >
                            ...
                            border border-gray-200 group'>
                                {imagePreview && <Image src={imagePreview} alt={editing.name} fill
                            ...
                            hover:bg-green-700 transition-all'onClick={handleEdit}
                                    disabled={loading}>{loading ? <Loader size={14} /> : "Edit Grocery"}</button>
                                <button className='px-4 py-2 rounded-lg bg-red-600 text-white flex items-center gap-2 
                            hover:bg-red-700 transition-all' onClick={handleDelete} disabled={deleteLoading}>
