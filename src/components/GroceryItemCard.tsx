...
function GroceryItemCard({ item }: { item: IGrocery }) {
    ...
    return (
        <motion.div
            ...
            viewport={{ once: false, amount: 0.2 }} className='bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden 
        border border-gray-100 flex flex-col'>
            ...
            <div className='p-4 flex flex-col flex-1'>
                ...
                {!cartItem ? <motion.div className='mt-4 flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700
                text-white rounded-full py-2 text-sm font-medium transition-all cursor-pointer' whileTap={{ scale: 0.96 }} onClick={() => dispatch(addToCart({ ...item, quantity: 1 }))}>
