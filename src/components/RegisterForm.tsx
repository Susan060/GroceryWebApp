...
function RegisterForm({ previousStep }: propType) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen px-6 py-10 bg-white relative">
      ...
      <p className='text-gray-600 flex items-center mb-2'>Join GroceryGo today <Leaf className='w-5 h-5 text-green-600' ></Leaf></p>
      <motion.form onSubmit={handleRegister} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }} className='flex flex-col gap-5 e-full max-w-sm'>
        ...
        {/* make the button div if any error arises */}
        <div className='w-full flex items-center justify-center gap-3 border border-gray-300 hover:bg-gray-100 py-3 rounded-xl text-gray-700 font-medium transition-all duration-200 cursor-pointer' onClick={()=>signIn("google",{callbackUrl:"/"})}>
          <Image src={googleImage} width={20} height={20} alt='google' />
          Continue with google
        </div>
      </motion.form>
      <p className='cursor-pointer text-gray-600 mt-6 tect-sm flex items-center gap-1' onClick={() => router.push("/login")}>Already have an account?<LogIn className='w-4 h-4' /><span className='text-green-600'>Sign in</span></p>
    </div>
  )
}
