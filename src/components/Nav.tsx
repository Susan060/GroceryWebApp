    ...
    const sideBar = menuOpen ? createPortal(
        <AnimatePresence>
            ...
            <motion.div initial={{ x: -100, opacity: 0 }}
                ...
                exit={{ x: -100 }}
                transition={{ type: "spring", stiffness: 100, damping: 14 }} className='fixed top-0 left-0 h-full w-[75%] sm:w-[60%] z-999 bg-linear-to-b from-green-800/90 via-green-700/80 to-green-900/90
            ...
            flex flex-col p-6 text-white' >
                ...
                <div className='flex items-center gap-3 p-3 mt-3 rounded-xl bg-white/10 hover:bg-white/15 transition-all shadow-inner'>
                    <div className='relative w-12 h-12 rounded-full overflow-hidden border-2 border-green-400/60 shadow-lg'>{user.image ? <Image src={user.image} alt='user' fill className='object-cover runded-full' /> : <User />}</div>
                    <div>
        ...
        <div className='w-[95%] fixed top-4 left-1/2 -translate-x-1/2 bg-linear-to-r from-green-500 to-green-700 rounded-2xl shadow-lg shadow-black/30 flex justify-between items-center h-15 px-4 md:px-8 z-50'>
            ...
            <div className='flex items-center gap-3 md:gap-6 relative'>
                {user.role == "user" && <><div className='bg-white rounded-full w-11 h-11 flex items-center justify-center shadow-md hover:scale-105 transition md:hidden' onClick={() => setSearchBarOpen((prev) => !prev)}>
                    ...
                    items-center justify-center rounded-full font-semibold shadow'>{cartData.length}</span></Link></>}
                ...
                <div className='relative' ref={profileDropDown}>
                    ...
                    <AnimatePresence>
                        {open && <motion.div initial={{ opacity: 0, y: -10, scale: 0.95 }}
                            ...
                            text-gray-700 font-medium' onClick={() => { setOpen(false); signOut({ callbackUrl: "/login" }) }}>
